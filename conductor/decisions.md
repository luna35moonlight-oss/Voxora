...
- **Amendment (Owner request, 2026-08-18):** the player's **Voxora avatar chooses the pet that races**. The avatar is read from the single Phase 3 avatar selection (no second avatar concept), the reserved meet records which avatar entered it, and a server-counted wins total is reported alongside the score. The Owner also shared a presentation of the intended look — a 3D race seen from behind the runners with a rewards rail. Pets-as-racers, the avatar choosing the racer, and the wins counter are implemented; the 3D presentation remains an ADR-004 art-pipeline task and the rewards rail stays unbuilt because rewards are an owner decision.
- **CORRECTION (Owner direction, 2026-08-18):** the earlier build was wrong on the central point and has been rewritten. **All four pets run continuously** under server simulation once a 3 → 2 → 1 → GO countdown ends; cards influence that running race (speed boosts, sprint, rival slowdowns, mud obstacles, shield) instead of creating the movement. Positions come from authoritative race progress, movement is continuous rather than tile-based, and the race is never paused for another screen. Checkpoint structure is **four normal checkpoints of 3 cards then a separate final checkpoint of 4** (8 + 3 + 3 + 3 + 3 + 4 = 24 cards a race). The **Voxora avatar enters a pet** and stays associated with it; the three rivals are Voxora house trainers and are never presented as real players. The **opening deal of 8 follows the chosen pet's race profile** rather than one universal hand, with per-pet weightings unapproved and marked as such. Racers are **development placeholders** that consume a pet identity, so Phase 4 Pet Foundation becomes the source of real pets rather than this game owning a second pet ecosystem; each species keeps its own silhouette and gait. Tactics are visible race events. The app no longer chooses a player's combination. Produced 3D/Rive pets, a following camera, and full per-species animation sets remain ADR-004 art-pipeline work; the reference presentation's rewards rail stays unbuilt because rewards are undefined.
- **Balance values:** no new owner rules were invented during the correction. Every unapproved number (course length, base pace, house pace variation, countdown, checkpoint positions, combination multipliers and durations, tactic strengths, mud size, score weights, race-end grace) is collected in `PET_CARD_RACE_BALANCE` in `packages/contracts` and reported for approval in `pet-card-race.md` §8.
